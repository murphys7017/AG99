...
class ChatUIProjectService:
    ...
    async def delete_project(self, username: str, project_id: str | None) -> None:
        project = await self._get_owned_project(username, project_id)
        await self.db.delete_chatui_project(project.project_id)
    ...
    async def add_session_to_project(self, username: str, data: object) -> None:
        ...
        await self.db.add_session_to_project(session_id, project.project_id)
    ...
    async def get_project_sessions(
        ...
        project = await self._get_owned_project(username, project_id)
        sessions = await self.db.get_project_sessions(project.project_id)
        return [self._serialize_session(session) for session in sessions]

    async def _get_owned_project(self, username: str, project_id: str | None):
        if not project_id:
    ...
    @staticmethod
    def _as_payload(data: object) -> dict[str, Any]:
    ...
    def _normalize_workspace_config(
        ...
        *,
        ...
        workspace_path = normalize_workspace_path(
            payload.get("workspace_path", fallback_path),
        )
