...
class LLMSummaryCompressor:
    ...
    Uses LLM to summarize old history while preserving recent whole rounds.
    """
    ...
    async def __call__(self, messages: list[Message]) -> list[Message]:
        ...
        Process:
        1. Divide messages by whole conversation rounds.
        2. Send the old messages + the instruction message to the LLM.
        3. Reconstruct: [system messages, summary pair, exact recent rounds].
        """
        ...
        # exact even if the ratio is 0 or the budget would summarize every round.
        ...
        if summary_contexts[-1].role != "assistant":
            summary_contexts.append(Message(role="assistant", content="Acknowledged."))
        ...
        # generate summary
        try:
            response = await self.provider.text_chat(contexts=sanitized_summary_contexts)
        ...
        # build result
        result = []
        result.extend(_extract_system_messages(messages))
        ...
        for rnd in recent_rounds:
            result.extend(seg for seg in rnd if isinstance(seg, Message))
