...
def hash_dashboard_password(raw_password: str) -> str:
    """Return a salted PBKDF2-HMAC-SHA256 hash for a dashboard password."""
...
def hash_legacy_dashboard_password(raw_password: str) -> str:
    """Return the legacy MD5 hash kept only for migration compatibility."""
    if not isinstance(raw_password, str) or raw_password == "":
        raise ValueError("Password cannot be empty")
...
def is_legacy_dashboard_password(stored: str) -> bool:
    return (
...
def is_pbkdf2_dashboard_password(stored: str) -> bool:
...
def verify_dashboard_password(stored_hash: str, candidate_password: str) -> bool:
    """Verify plaintext password against legacy MD5 or PBKDF2-SHA256 storage."""
    if not isinstance(stored_hash, str) or not isinstance(candidate_password, str):
        return False

    if is_legacy_dashboard_password(stored_hash):
    ...
    if is_pbkdf2_dashboard_password(stored_hash):
