...
class InteractionDecisionAgent:
    ...
    async def decide(
        ...
        event.set_extra("_interaction_prompt_render_result", render_result)
        logger.debug(
            "Interaction decision context built: platform_id=%s session_id=%s persona_keys=%s memory_keys=%s recent_messages=%s tools_available=%s tool_count=%s prompt_extensions=%s rendered_slots=%s",
            event.get_platform_id(),
            event.session_id,
            ...
            material.prompt_context_pack.meta.get(
                "interaction_prompt_extension_count", 0
            ),
...
def add_interaction_decision_slots_to_pack(
    *,
    pack,
    event,
    ...
    extensions = [
        PromptExtension(
            ...
            mount="system",
            title="Interaction middleware decision policy",
            value_kind="text",
            value=build_interaction_agent_system_prompt(),
            order=0,
            meta={
                "scope": "static",
