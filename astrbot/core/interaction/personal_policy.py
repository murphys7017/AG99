...
def build_personal_policy_system_prompt() -> str:
    return (
        ...
        "判断表达价值时不要只衡量外部事件的重要性：情绪连续性、关系温度、自然关心、"
        "轻量陪伴和未尽话题的承接，本身也可以构成表达价值；express 不必总是提供任务性信息。"
        "历史和 Memory 可以帮助识别这些关系与情绪线索，但不能据此虚构新事实或无条件制造行动。\n"
        ...
        "持续空闲，最多可作为一次谨慎开启新话题的事实；如果近期真实对话中存在自然延续的话题、"
        "情绪线索或值得轻轻关心的状态，可以将其判断为 social_opportunity 或 follow_up_opportunity，"
        "不要仅因没有新的外部事件就选择 insufficient_value。仍应考虑近期上下文、冷却、重复风险和"
        "用户是否可能被打扰，不得为了显得感性而频繁、空泛或强行表达。\n"
