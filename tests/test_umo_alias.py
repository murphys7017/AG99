...
def test_umo_alias_is_in_backup_model_list():
    assert MAIN_DB_MODELS["umo_aliases"] is UmoAlias
...
@pytest.mark.asyncio
async def test_session_route_includes_alias_only_umos(temp_db):
    from astrbot.dashboard.routes.session_management import SessionManagementRoute

    route = SessionManagementRoute.__new__(SessionManagementRoute)
    route.db_helper = temp_db
    route._get_groups = lambda: {}
    ...
    umos = await route._list_known_umos()
    alias_map = await route._get_umo_alias_map(umos)
    info = route._build_umo_info("qq:GroupMessage:1000", alias_map)

    assert umos == ["qq:GroupMessage:1000"]
    assert info["platform"] == "qq"
    assert info["message_type"] == "GroupMessage"
    assert info["session_id"] == "1000"
    assert info["display_name"] == "Backend Room"
