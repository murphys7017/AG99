...
async def test_send_message_missing_image_path_stops_before_send(tmp_path, monkeypatch):
    ...
    async def mock_get_booter(*args, **kwargs):
        del args, kwargs
        raise FileNotFoundError(f"image path does not exist: {missing_image_path}")
    ...
    assert f"error: image path does not exist: {missing_image_path}" in result
...
async def test_non_admin_can_send_workspace_file(tmp_path, monkeypatch):
    ...
    workspace_file = (
        workspace_base / normalize_umo_for_workspace(current_session) / "result.txt"
    )
