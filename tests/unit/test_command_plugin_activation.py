"""Commands from disabled plugins should expose activation separately."""
...
def test_command_summary_treats_inactive_plugin_commands_as_disabled():
    assert is_command_effectively_enabled(
        {"enabled": True, "plugin_activated": True}
    )
    assert not is_command_effectively_enabled(
        {"enabled": True, "plugin_activated": False}
    )
    assert not is_command_effectively_enabled(
        {"enabled": False, "plugin_activated": True}
    )
    assert is_command_effectively_enabled({"enabled": True})
