...
@pytest.mark.asyncio
async def test_mcp_client_connect_and_cleanup_run_in_same_lifecycle_task(monkeypatch):
    instances = []
    ...
    class FakeMCPClient:
        ...
        async def connect_to_server(self, _config, _name):
    ...
    monkeypatch.setattr(ftm, "MCPClient", FakeMCPClient)

    manager = FunctionToolManager()
    await manager.enable_mcp_server("fake", {"url": "https://example.com/mcp"})

    assert manager.get_func("fake_tool") is not None

    await manager.disable_mcp_server("fake")

    client = instances[0]
    assert client.connect_task is not None
    assert client.cleanup_task is client.connect_task
    assert manager.get_func("fake_tool") is None
