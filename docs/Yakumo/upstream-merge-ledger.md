...
| --- | --- | --- | --- | --- |
| Security fixes | In progress | Upload path traversal, backup importer traversal, password policy, updater zip root path, T2I SSTI validation | Upload filename sanitization, backup-importer handling, updater zip-root handling, and T2I template validation are already present. Password policy remains deferred because upstream `7ddf6371` is a broad auth/onboarding/storage migration, not a small patch. | Keep PBKDF2/setup as a dedicated auth migration batch if this fork chooses to change the dashboard login protocol. |
...
| `93428a79` | Core initial dashboard password env | Deferred | Upstream wires env resolution into full config initialization with PBKDF2/setup semantics. This fork only absorbed the CLI-init subset to avoid changing runtime config creation or dashboard login protocol. |
| `7ddf6371` | Dashboard password policy/PBKDF2 setup flow | Deferred | Broad backend/WebUI/auth storage migration touching login protocol, setup route, stat route, password state, and frontend auth flow. Keep out of low-risk sync batches. |
| `e05dd650` | Legacy password login guidance | Deferred | Tied to the upstream PBKDF2 legacy-login failure mode, which this fork has not adopted. Current local login still compares the WebUI-sent MD5 value to `dashboard.password`. |
