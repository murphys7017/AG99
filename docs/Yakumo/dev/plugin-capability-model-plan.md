...
状态：**提案 / 已批准实施**。第 1–4 阶段已落地（见 §10 实施记录），其余阶段进行中。
未改动 `plugin_runtime_targets`、`plugin_tool_targets` 的现有语义。
...
| `Context.register_web_api` / `registered_web_apis` | `context.py:2029` 写入的是**类属性**，`dashboard/server.py:205-207` 消费；无 activated / plugins_name 检查 | **确认**：插件停用后路由仍可命中；类属性共享还意味着跨 Context 实例可见 |
