...
- consolidation 与长期 promotion 已由后台 scheduler 承担；Recall 刷新、dirty vector sync 和 PersonaState 自动演进仍未接入统一 scheduler。
...
2. 将 Recall 刷新与 dirty vector sync 接入统一的预算、调度和任务 owner。
3. 完善 Memory read/write latency、降级组件和后台任务诊断。
4. 固化 finalized material 到 MemoryUpdateRequest 的版本化契约，并规划 PersonaState 自动演进。
