...
- Interaction turn 中的插件 LLM 生命周期和插件拥有的工具默认挂载到 Persona Expression；运行目标依次由 `interaction_middleware.plugin_runtime_targets` 配置、插件的 `interaction_runtime_target` 声明和 Persona 默认值解析，最终为 `core` 的插件才进入 Core。Persona 工具的旧式可见输出会成为模型可见工具材料，最终 Persona Expression 独占用户可见回复。Pipeline Handler 保持原有位置和终止事件语义，不被迁移为 Persona 插件。
