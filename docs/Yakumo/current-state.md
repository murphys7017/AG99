...
- `PersonalState` 还维护进程内 material revision。真实用户活动、无 coalesce identity 的 Observation，以及同一 Sensor identity 下 payload 发生变化的事实会推进 revision；Heartbeat 只是时钟信号，不推进 revision。Policy 形成 `express` intent 时在调用 Persona/平台前消费本次 Gate 快照对应的 revision，因此发送失败仍不写冷却或配额，但同一事实不会在下一次 Heartbeat 重跑 Policy、Persona 和发送。调用期间到达的新事实使用更高 revision，不会被当前 intent 误消费。成功表达的持久 `last_expression_at` 会在重启恢复时阻止无新事实 Heartbeat 立即重放；Sensor payload 指纹和失败尝试 revision 保持进程内。
...
- `Context.send_message()` 的主动纯文本输出进入 Personal Runtime；当前 session 的 Core 工具输出作为 progress，跨 session 输出建立独立 proactive turn。显式支持 Personal Runtime 的 Observation 输出会按逻辑 TTS message ID 保留 Record 与双输出 Plain 的复合消息链，因此一次自主表达只建立一个 proactive turn；其他投递仍保持 Record 独立发送兼容行为。上一条回复防重只限 Policy 形成的 `PersonalActionIntent` 自主表达，不改写或抑制 `Context.send_message()`、Cron 和插件显式主动发送。assistant-only 输出以空 `user_message` 为规范表示，可进入后续 Prompt 与 Memory history；非空但缺少身份的媒体或畸形用户载荷仍保留 warning。
- 主动 Observation 的 Output Controller 元数据通过 `Platform.send_by_session_with_extras()` 传到支持 richer payload 的 Adapter；基类默认调用原有 `send_by_session()`，因此 Context、Cron、插件和既有 Adapter 的公开发送方式不变。复合 TTS 的 logical message ID、segment 和 TTS 状态不再在 `Context._send_message_direct()` 丢失。
...
- `router_agent` 保持现有轻量路由职责：普通显式唤醒只判断 `persona` / `hybrid`；仅 Personal Runtime 标记的群聊模型续接候选增加 `silent`，用于判断当前未唤醒消息是否确实承接 Bot 最近回复。该候选在 Waking 阶段不扫描或激活普通插件 Handler，先进入 ProcessStage 的 Router；未承接必须 `silent`，只有确认承接后才在 `persona / hybrid` 间选择。Router 不生成用户回复、不注册 tool-call、也不输出 effect。直播音频和协议命令走独立 Core bypass，不伪装成 Router 结果。Router 只消费规范 `ContextPack` 的极简投影，不参与事实采集。
- 普通消息在输入完成 materialization 后仍并发启动 Router 与 Persona Expression。群聊模型续接候选先等待 Router；`silent` 直接零输出完成，`persona / hybrid` 才启动 Persona，Router 失败也 fail closed 为 `silent`。Turn State 继续用 `pending / committed / emitted / suppressed / failed` 仲裁输出。
...
- `platform_settings.personal_runtime_observation_targets` 可以显式选择多个 Personal Runtime 观察目标；留空时兼容使用 `proactive_message_target`，且不改变无目标主动消息的发送位置。Context 始终从默认配置解析这一份全局观察范围；Heartbeat 按每个目标实际命中的 Runtime 配置读取开关与间隔，并为每个启用目标维护独立 due time，只调用通用 `submit_observation()`；群聊环境观察默认关闭，启用后仅放行该范围内、且当前会话配置已开启功能的非唤醒群聊文本，经官方白名单和会话状态检查后转换为不含原文的 `conversation_activity` fact，并在进入限流、插件、Router 和 Core 前停止原事件。两类 Source 都不构造平台事件、不直接调用 Persona/Core/Output。插件可通过 `Context.register_runtime_observation_sensor()` 注册受限的结构化事实来源；Context 只解析目标并经 Lifecycle dispatcher 交给已有 Runtime Manager，注册随插件卸载清理。
- 群聊历史上下文本身不授予隐式唤醒权限。当前 active turn 的同一发送者可立即 follow-up；Bot 成功回复后的前 10 秒允许该发送者直接续接，此后到 `personal_runtime_conversation_continuation_seconds` 截止只作为模型续接候选，由现有 Router 在 `silent / persona / hybrid` 中判断。该候选不运行普通插件 Handler；窗口外、其他发送者以及 Router 判断 `silent` 的消息不进入对话，Router 失败同样静默关闭。
