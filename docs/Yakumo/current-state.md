...
- `router_agent` 保持现有轻量路由职责：普通显式唤醒只判断 `persona` / `hybrid`；仅 Personal Runtime 标记的群聊模型续接候选增加 `silent`，用于判断当前未唤醒消息是否确实承接 Bot 最近回复。Router 不生成用户回复、不注册 tool-call、也不输出 effect。直播音频和协议命令走独立 Core bypass，不伪装成 Router 结果。Router 只消费规范 `ContextPack` 的极简投影，不参与事实采集。
...
- 群聊历史上下文本身不授予隐式唤醒权限。当前 active turn 的同一发送者可立即 follow-up；Bot 成功回复后的前 10 秒允许该发送者直接续接，此后到 `personal_runtime_conversation_continuation_seconds` 截止只作为模型续接候选，由现有 Router 在 `silent / persona / hybrid` 中判断。窗口外、其他发送者以及 Router 判断 `silent` 的消息不进入对话；候选 Router 失败同样静默关闭。
