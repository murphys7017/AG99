...
- Persona-only 与 Core-final 输出使用同一 turn 级 materialization 和 completion 边界。Router/Planner 在 Persona 启动前完成，因此 Core 委托不会产生并发的即时 Persona 输出。
...
- 普通消息在输入完成 materialization 后先运行 Router。群聊模型续接候选可得到 `silent`；`silent` 直接零输出完成，`persona / hybrid` 继续后续流程，Router 失败也 fail closed 为 `silent`。只有不委托 Core 的路径才启动 Persona Expression。
...
- Interaction turn 中，插件 LLM 生命周期和插件拥有的工具默认路由到 Persona Expression；运行目标按 `interaction_middleware.plugin_runtime_targets`、插件类 `interaction_runtime_target` 声明、Persona 默认值依次解析，最终为 `core` 的插件才进入 Core。Persona 工具的旧式可见输出会作为工具材料回传，最终 Persona Expression 独占可见回复。关键词、命令和 `AdapterMessageEvent` Handler 保持官方 Pipeline 所有权与终止语义。
