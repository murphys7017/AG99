...
- 群聊历史上下文本身不授予隐式唤醒权限。当前 active turn 的同一发送者可立即 follow-up；Bot 成功回复后的前 10 秒允许该发送者直接续接，此后到 `personal_runtime_conversation_continuation_seconds` 截止只作为模型续接候选，由现有 Router 在 `silent / persona / hybrid` 中判断。候选在 Router 前不运行 Handler，只有 Router 接受后才恢复一次既有 Handler 发现；窗口外、其他发送者以及 Router 判断 `silent` 的消息不进入对话，Router 失败同样静默关闭。
