...
The Yakumo fork in this repository adds Interaction Middleware between the official EventBus/Pipeline and Core Agent. For normal conversations, a Router with no tools or JSON contract selects `silent`, `persona`, or `hybrid`; a hybrid turn is checked by Core Planner before it either delegates to Core or enters the single Persona Runtime, and Core results return through that same expression layer. Plugins enhance Persona Expression by default and enter work execution only when their configuration or `interaction_runtime_target` declaration resolves to `core`. [Persona Effects](/en/dev/star/guides/persona-effects) add structured presentation output, while [Prompt Extensions](/en/dev/star/guides/prompt-extensions) contribute target-scoped model facts without leaking platform-specific semantics into Router or Core.
