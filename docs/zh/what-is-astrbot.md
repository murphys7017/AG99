...
当前仓库的 Yakumo fork 在官方 EventBus / Pipeline 与核心 Agent 之间增加 Interaction Middleware。普通对话先由无工具、无 JSON 契约的 Router 选择 `silent` / `persona` / `hybrid`；`hybrid` 先由 Core Planner 判断是否执行，未委托 Core 的路径直接进入唯一 Persona Runtime，Core 结果也回到该表达层。插件默认增强 Persona Expression，只有配置为 `core` 的插件进入工作执行；[Persona Effect](/dev/star/guides/persona-effects) 与 [Prompt Extension](/dev/star/guides/prompt-extensions) 分别扩展结构化表现和目标明确的模型事实，而不把平台私有语义写入 Router 或 Core。
