...
为了避免在处理 `@` 消息时同步请求 OneBot，AstrBot 会在适配器连接后后台获取可用群列表，并异步预热群成员昵称缓存。缓存有效期为 24 小时，刷新任务会限制并发，不会阻塞消息接收和回复主链路。

如果缓存尚未完成或 OneBot 暂时无法返回成员列表，`@` 消息会回退使用消息段中携带的昵称，缺少昵称时使用 QQ 号。群成员变更通知会触发对应群的后台刷新；启用 ID 白名单时，只会预热白名单允许的群。
